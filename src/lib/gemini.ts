...
6. List 3 specific use cases for their daily workflow.
7. Provide a "comparisonStrategy": Explain that they should take their best prompt, run it in both the primary tool and the alternative tools, and compare the outputs to see which one gives the best response for their specific style.
8. Give 1 pro tip.
9. For the "nextStep", explicitly state that knowing the tool is only the beginning, and they need the AI Literacy Academy to master the actual workflows and strategies.
`;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`Starting AI generation with model: ${modelName} (Attempt ${attempt + 1})`);
      ...
      if (error.message?.includes("leaked") || String(error).includes("leaked")) {
        throw new Error("SECURITY_ALERT: Your Gemini API Key has been reported as leaked by Google. For your security, this key has been deactivated. Please generate a NEW API key in Google AI Studio and update it in your app's Secrets setting.");
      }
  ...
  throw new Error(lastError?.message || "Failed to generate AI recommendation. Please try again.");
}
